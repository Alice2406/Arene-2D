...
Sniper::Sniper(SniperSkin skinType, sf::Texture& texture) : m_sprite(m_texture), m_animator(m_sprite)
{
...
void Sniper::setPosition(const sf::Vector2f& position)
{
    m_sprite.setPosition(position);
}

sf::Sprite& Sniper::getSprite() 
{ 
    return m_sprite; 
}

void Sniper::Update(float dt)
{
    fsm.Update(context);
...
void Sniper::handleDamage(float amount)
{
...
bool Sniper::IsDead() const
{
    return health.IsDead();
}

sf::Vector2f Sniper::getPosition() const 
{ 
    return m_sprite.getPosition(); 
}

void Sniper::SetProjectileTexture(sf::Texture& tex) 
{
